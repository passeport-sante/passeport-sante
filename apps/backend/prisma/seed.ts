...
async function main() {
  ...
  // ─── Mascottes ────────────────────────────────────────────────────────────────
  // Catalogue initial : les visuels livrés dans public/assets/mascotte/. Ils sont
  // référencés par nom de fichier, pas par URL Cloudinary — le helper mascotteUrl()
  // côté front gère les deux formats. Les mascottes ajoutées par les admins depuis
  // le back-office arrivent ensuite avec une URL Cloudinary.
  const seedMascottes = [
  ...
  console.log(` ${seedMascottes.length} mascottes créées`);
